...
# 1. NASTAVITVE (Ohranjen tvoj naslov)
st.set_page_config(page_title="MESTNA POEZIJA", page_icon="🎨")

# 2. TVOJE PERFEKCIONISTIČNE TEME (Nič spremenjeno)
TEME = {
...
# 3. DODATEK ZA STABILNOST (Cache, da ne crkne ob 15k ogledih)
@st.cache_data(show_spinner="Ustvarjam vaš A4 poster...", max_entries=20)
def ustvari_poster(mesto, drzava, razdalja, ime_teme):
    try:
        ...
        # network_type="all" kot si želel, a s cacheom bo delalo hitreje
        ...
        # DODANE KOORDINATE (Točno pod državo)
        ...
        buf.seek(0)
        plt.close(fig) # Sprostimo RAM
        return buf.getvalue()
    except Exception as e:
        return str(e)

# 4. UI (Tvoj originalen razpored)
...
if st.button("✨ GENERIRAJ MOJSTROVINO"):
    rezultat = ustvari_poster(mesto_vnos, drzava_vnos, zoom_vnos, tema_vnos)
    if isinstance(rezultat, bytes):
        st.image(rezultat, use_container_width=True)
        st.download_button("📥 PRENESI A4 POSTER (PNG)", rezultat, file_name=f"{mesto_vnos}_A4.png")
    else:
        st.error(f"Napaka: {rezultat}")

# 5. TVOJ PAYPAL DEL (Nič spremenjeno)
st.write("---")
st.markdown(
    """
