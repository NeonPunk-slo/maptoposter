...
def ustvari_poster(mesto, drzava, razdalja, ime_teme):
    ...
    for u, v, k, data in G.edges(data=True, keys=True):
        ...
        if isinstance(h_type, list): h_type = h_type[0] # Če je več tipov, vzemi prvega
...
st.markdown("<h1 style='text-align: center; color: #333;'>🎨 Mestna Poezija</h1>", unsafe_allow_html=True)
...
if st.button("✨ Ustvari poster"):
    with st.spinner("Rišem digitalne poti in iščem morje..."):
        try:
            ...
            st.error(f"Prišlo je do napake pri generiranju. Poskusi z manjšim zoomom. Napaka: {e}")
