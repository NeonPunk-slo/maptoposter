...
def dobi_koordinate(mesto, drzava):
    try:
        geolocator = Nominatim(user_agent="mestna_poezija_v2026_final")
...
if st.button("✨ Ustvari poster"):
    with st.spinner("Pripravljam umetniško delo..."):
        rezultat = ustvari_poster(mesto, drzava, razdalja, izbrana_tema)
        if rezultat:
            st.image(rezultat, use_container_width=True)
            st.download_button(label="📥 Prenesi poster", data=rezultat, file_name=f"{mesto}.png")
